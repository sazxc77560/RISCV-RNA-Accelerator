...
double calculateCharCountStdDev(const std::vector<std::vector<std::string>>& data, double& mean, int& largest_partition_idx) {
	...
	if (count-mean > largest_deviation) {
	    largest_deviation = count-mean;
...
void distribute(
    const std::vector<std::string>& transcript_sequences,
    ...
    auto partition_sequences = [&](const std::vector<std::string>& sequences,
                                   std::vector<std::vector<std::string>>& partitions) {
        int total = sequences.size();
        int base = total / logic_unit_count;
        int rem = total % logic_unit_count;

        partitions.clear();  
        partitions.reserve(logic_unit_count);

        int start = 0;
        for (int i = 0; i < logic_unit_count; ++i) {
            int len = base + (i < rem ? 1 : 0);
            partitions.emplace_back(sequences.begin() + start, sequences.begin() + start + len);
            start += len;
        }
    };

    partition_sequences(transcript_sequences, transcript_sequence_partitions);
    partition_sequences(query_sequences, query_sequence_partitions);
}
