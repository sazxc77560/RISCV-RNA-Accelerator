...
void quantify(HashMap *map, const char **query_sequences_used, int query_sequences_used_count, int k, int *final_results) {
    char *kmer_buf = malloc(k + 1);
    kmer_buf[k] = '\0';

    int *counts = calloc(index_sequences_count, sizeof(int));
    int *last_updated = calloc(index_sequences_count, sizeof(int));
    int round = 1;

    IntArray matched_transcripts_idxs;
    init_int_array(&matched_transcripts_idxs);

    for (int i = 0; i < query_sequences_used_count; ++i, ++round) {
        const char *seq = query_sequences_used[i];
        int len = strlen(seq);
        int kmer_count = len - k + 1;
        int max_count = 0;

        for (int j = 0; j < kmer_count; ++j) {
            
            strncpy(kmer_buf, seq + j, k);
            matched_transcripts_idxs.size = 0;
            get_values(map, kmer_buf, &matched_transcripts_idxs);

            for (int l = 0; l < matched_transcripts_idxs.size; ++l) {
                int tid = matched_transcripts_idxs.data[l];

                
                if (last_updated[tid] != round) {
                    last_updated[tid] = round;
                    counts[tid] = 1;
                } else {
                    counts[tid]++;
                }

                if (counts[tid] > max_count)
                    max_count = counts[tid];
            }
        }

        if (max_count > 0) {
            for (int j = 0; j < index_sequences_count; ++j) {
                if (last_updated[j] == round && counts[j] == max_count)
                    final_results[j]++;
            }
        }
    }
    ...
    free(counts);
    free(last_updated);
    free(matched_transcripts_idxs.data);
}
