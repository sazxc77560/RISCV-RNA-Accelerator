...
void generate_index(const char **index_sequences, int index_sequences_count, int k, HashMap *map){
    ...
    for (int i = 0; i < index_sequences_count; i++) {
        ...
        for (int p = 0; p < k - 1; p++) base_k = (base_k * base) % mod;

        
        for (int j = 0; j < k; j++) {
            hash = (hash * base + seq[j]) % mod;
        }

        
        char *kmer_buf = (char *)malloc(k + 1);
        kmer_buf[k] = '\0';
        memcpy(kmer_buf, seq, k);
        insert(map, kmer_buf, i);

        
        for (int j = 1; j <= len - k; j++) {
            
            hash = (hash + mod - (seq[j - 1] * base_k) % mod) % mod;
            hash = (hash * base + seq[j + k - 1]) % mod;

            
            memcpy(kmer_buf, seq + j, k);
            insert(map, kmer_buf, i);
        }
        free(kmer_buf);
    }
